...
function login(requiredFields, entries, data, next) {
  ...
  var eventsPageUrl = baseUrl + 'mon-compte/rendez-vous';
  ...
  request(options, function (err, res, body) {
    if (err) {
      ...
      request(options, function (err, res) {
        ...
        if (res.statusCode === 302 && res.headers.location === baseUrl + 'sessions/nouveau') {
...
function saveEvents(requiredFields, entries, data, next) {
  ...
  entries.nbUpdates = 0;
  async.eachSeries(entries.events, function (icalEvent) {
