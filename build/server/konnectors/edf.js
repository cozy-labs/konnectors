...
var Bill, Client, ConsumptionStatement, Contract, File, Home, K, PaymentTerms, _extend, async, buildNotifContent, builder, cozydb, createNewFile, displayData, edfRequestPost, fetchEdeliaElecIndexes, fetchEdeliaGasIndexes, fetchEdeliaMonthlyElecConsumptions, fetchEdeliaMonthlyGasConsumptions, fetchEdeliaProfile, fetchEdeliaSimilarHomeYearlyElecComparisions, fetchEdeliaSimilarHomeYearlyGasComparisions, fetchEdeliaToken, fetchListerContratClientParticulier, fetchPDF, fetchRecupereDocumentContractuelListx, fetchVisualiserAccordCommercial, fetchVisualiserCalendrierPaiement, fetchVisualiserHistoConso, fetchVisualiserPartenaire, getEDFToken, getEdelia, getF, logger, moment, parser, prepareEntries, request, saveMissingBills, translate, updateOrCreate, xml2js,
  slice = [].slice;
...
getEDFToken = function(requiredFields, entries, data, callback) {
  ...
  return edfRequestPost(path, body, function(err, result) {
    ...
    if (token != null) {
      ...
      return callback(new Error("Can't fetch token"));
    }
  });
};
...
fetchVisualiserAccordCommercial = function(requiredFields, entries, data, callback) {
  ...
  return edfRequestPost(path, body, function(err, result) {
    var acoElem, bankAddress, e, paymentTerms, services, servicesElem;
    if (err) {
      return callback(err);
    }
    try {
      ...
      entries.apymenttermss.push(paymentTerms);
...
fetchVisualiserCalendrierPaiement = function(requiredFields, entries, data, callback) {
  ...
  return edfRequestPost(path, body, function(err, result) {
    ...
    try {
      ...
      if (!entries.apymenttermss[0]) {
        entries.apymenttermss[0] = {
          vendor: 'EDF',
          clientId: entries.clients[0].clientId,
          docTypeVersion: K.docTypeVersion
        };
      }
      entries.apymenttermss[0].paymentSchedules = paymentSchedules;
...
buildNotifContent = function(requiredFields, entries, data, next) {
  ...
  return entries.notifContent = addedList.join(', ');
};
...
K = module.exports = require('../lib/base_konnector').createNew({
  ...
  init: function(callback) {
    return async.each([Client, Contract, PaymentTerms, Home, ConsumptionStatement], function(docType, cb) {
      return docType.defineRequest('all', cozydb.defaultRequests.all, cb);
    }, function(err) {
      if (err) {
