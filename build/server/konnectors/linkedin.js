...
var ACCOUNT_TYPE, CompareContacts, Contact, ContactHelper, Tag, async, cheerio, fetcher, https, linkedin, localization, log, logIn, prepareCozyContacts, request, retrieveContactList, retrieveTokens, saveContact, saveContacts, savePicture, updateContact, url;
...
module.exports = {
  ...
  fetch: function(requiredFields, callback) {
    ...
    return fetcher["new"]().use(retrieveTokens).use(logIn).use(retrieveContactList).use(prepareCozyContacts).use(saveContacts).args(requiredFields, {}, {}).fetch(function(err, fields, entries) {
      var localizationKey, notifContent, options, stats;
      if (err) {
        return callback(err);
      }
...
saveContact = function(newContact, entries, callback) {
  ...
  if (entries.cozyContactsByAccountIds[linkAccount.id] != null) {
    ...
    if (newRev !== previousRev) {
      ...
      return updateContact(cozyContact, newContact, imageUrl, entries.contactStats, callback);
    } else {
...
updateContact = function(fromCozy, fromLinkedin, imageUrl, contactStats, callback) {
  var newRev;
  CompareContacts.mergeContacts(fromCozy, fromLinkedin);
  newRev = ContactHelper.intrinsicRev(fromCozy);
  return fromCozy.save(function(err, saved) {
    if (err) {
      return callback(err);
    }
