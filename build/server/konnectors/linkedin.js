...
module.exports = {
  ...
  fetch: (requiredFields, callback) => {
    ...
    Promise.resolve(requiredFields).then(retrieveTokens).then(logIn).then(retrieveContactList).then(prepareCozyContacts).then(getOrCreateTag).then(retrieveAndSaveContacts).then(createNotificationContent).then(notifContent => {
...
function retrieveContactData(data) {
  return new Promise((resolve, reject) => {
    ...
    const contactUrl = 'https://www.linkedin.com/contacts/api/contacts/' + `${ data.contact.id }/?fields=name,first_name,last_name,` + 'emails_extended,phone_numbers,sites,addresses,' + 'company,title,geo_location,profiles,twitter,tag,' + ' secure_profile_image_url';
...
function saveContacts(arrayContacts, data) {
  return new Promise((resolve, reject) => {
    arrayContacts.forEach(contact => {
      const linkAccount = ContactHelper.getAccount(contact, ACCOUNT_TYPE, data.entries.accountName);
