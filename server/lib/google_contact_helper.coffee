...
GCH.fromGoogleContact = (gContact, accountName)->
    ...
    "#{nameComponent('gd$givenName')};#{nameComponent('gd$additionalName')};" +
    "#{nameComponent('gd$namePrefix')};#{nameComponent('gd$nameSuffix')}"
...
GCH.toGoogleContact = (contact, gEntry) ->
    ...
    name.gd$additionalName = $t: middleName if middleName?\
        and middleName isnt ''
    ...
    req = https.request options, (res) ->
        ...
        res.on 'data', (chunk) ->
            if res.statusCode isnt 200
                log.info "#{res.statusCode} while uploading" +
                " picture: #{chunk.toString()}"
            callback()
